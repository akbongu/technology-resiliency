...
- Support provider validation using cloud CLI profile, subscription, tenancy checks, or environment-based credentials.
- Support provider authentication through a modular cloud-provider adapter for AWS, Azure, GCP, and OCI, with credentials sourced from environment variables when configured.
